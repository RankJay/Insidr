...
def userMetricComponent(userID):
    ...
    scrappedValues = [{"created_at": unscrappedInfo['data'][0]['created_at']}, {"name": unscrappedInfo['data'][0]['name']}, {"username": unscrappedInfo['data'][0]['username']}]
    return scrappedValues

def tweetMetricComponent(tweetID):
    unscrappedInfo = TwitterAPI.tweetMetrics(tweetID)
    scrappedValues = [{"author_id": unscrappedInfo['data'][0]['author_id']}, {"created_at": unscrappedInfo['data'][0]['created_at']}, {"public_metrics": unscrappedInfo['data'][0]['public_metrics']}]
...
def createInstance(PARENT_TWEET_ID):
    ...
    Collectibles(utilityInstance.retweetersScraping())

createInstance(1400741886529179653)